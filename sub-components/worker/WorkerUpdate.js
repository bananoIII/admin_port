...
const WorkerUpdate = ({workerId}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Solo incluye password si fue cambiado
    ...
    try {
      ...
      router.push("/admin/workers"); // Redirige si todo va bien
    } catch (error) {
      console.error(error);
  ...
  useEffect(() => {
    ...
    const fetchWorker = async () => {
      try {
        ...
        // Contacto
        setContactEmail(worker.contact.email);
        setPhone(worker.contact.phone);
  
        // Dirección
        setStreet(worker.address.street);
        setCity(worker.address.city);
        setState(worker.address.state);
        setCountry(worker.address.country);
        setZipCode(worker.address.zipCode);
  
        // Trabajo
        setPosition(worker.jobInfo.position);
        setSalary(worker.jobInfo.salary);
        setSeniority(worker.jobInfo.seniority);
        setWorkStartTime(worker.jobInfo.workStartTime);
        setWorkEndTime(worker.jobInfo.workEndTime);
  
        // Usuario del sistema
        setSystemEmail(user?.email || "");
  
        // Si quieres dejar el password vacío (editable opcional)
        setPassword("");
  ...
  const handleFire = async (id) => {
    const confirm = window.confirm('¿Estás seguro de que quieres despedir a este trabajador?');
    if (!confirm) return;
    ...
    try {
      ...
      alert('Ocurrió un error al despedir al trabajador');
    }
  };
      ...
      <Row className="mb-8">
        <Col xl={12}>
          <Card>
            <Card.Body>
              <div className="mb-6">
                <h4 className="mb-1">Configurar trabajador</h4>
              </div>
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Row>
                  ...
                  <Button variant="danger" onClick={() => handleFire(id)}>
                      Despedir
                    </Button>
