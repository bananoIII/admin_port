...
const [modelo, setModelo] = useState("");
  ...
  const [asientosNormales, setAsientosNormales] = useState("");
  const [asientosPremium, setAsientosPremium] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const planeData = {
      ...
      estado // si lo manejas como input tipo select o text
    };
    ...
    try {
      ...
      if (response.ok) {
        router.push('/admin/plane'); // ajusta la ruta según tu app
      } else {
...
useEffect(() => {
  ...
  const fetchPlane = async () => {
    ...
    setAsientosNormales(data.asientos.normales);
    setAsientosPremium(data.asientos.premium);
    setPesoMaximoKg(data.peso_maximo_kg);
